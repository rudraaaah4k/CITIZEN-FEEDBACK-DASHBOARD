      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground/90">Pin the location on a map (optional)</label>
              <LocationPicker
                value={coordinates}
                onChange={(loc) => {
                  setCoordinates({ lat: loc.lat, lng: loc.lng });
                  setValue('address', loc.address || loc.formattedAddress, { shouldValidate: false });
                  if (loc.city) setValue('city', loc.city);
                  if (loc.state) setValue('state', loc.state);
                  if (loc.pincode) setValue('pincode', loc.pincode);
                }}
              />
            </div>
