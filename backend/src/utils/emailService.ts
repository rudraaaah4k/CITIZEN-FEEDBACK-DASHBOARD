...
export const sendOtpEmail = async (
  ...
  const subject = isVerify ? 'Verify Your Email - CitizenFeedback' : 'Password Reset Code - CitizenFeedback';
  ...
  const html = `
    ...
    <head>
      <style>
        ...
        .card { background: #1a1a2e; border: 1px solid #2d2d3a; border-radius: 16px; padding: 40px; }
        .otp-box { background: #0f0f1a; border: 1px solid #6366f1; border-radius: 12px; padding: 24px; margin: 24px 0; text-align: center; }
        .otp-code { font-size: 36px; font-weight: 800; letter-spacing: 10px; color: #a5b4fc; font-family: 'Courier New', monospace; }
    ...
    <body>
      <div class="container">
        ...
        <div class="card">
          ...
          <div class="warning">
            ⚠️ If you did not request this, please ignore this email or contact support if you're concerned.
          </div>
        </div>
      </div>
    </body>
  ...
  await sendEmail({ to: email, subject, html, text: `Your verification code is ${otp}. It expires in 10 minutes.` });
};
